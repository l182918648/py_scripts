...
class QcloudApi():
  def __init__(self):
    ...
    SECRETID = os.environ["QCLOUD_SECRET_ID"]  #'AKIDV**********Hfo8CzfjgN'
    SECRETKEY = os.environ["QCLOUD_SECRET_KEY"]   #'ZrVs*************gqjOp1zVl'
  ...
  def del_record(self, domain, record):
    ret = self.get(module = 'cns', action = 'RecordDelete', domain = domain, recordId = record)
    #替换为标准格式
    # {
    #   "result":True,
    #   "message":{...}
    # }
    data = {}
    data["result"] = ret.get('code') == '0'
    data["message"] = retget('message')
    return data
  ...
  def create_record(self, domain, sub_domain, value, record_type, line):
    ret = self.get(module = 'cns', action = 'RecordCreate', domain = domain, subDomain = sub_domain, value = value, recordType = record_type, recordLine = line)
    #替换为标准格式
    # {
    #   "result":True,
    #   "message":{...}
    ...
    data = {}
    data["result"] = ret.get('code') == '0'
    data["message"] = ret.get('message')
    return data

  def change_record(self, domain, record_id, sub_domain, value, record_type, line):
    ret = self.get(module = 'cns', action = 'RecordModify', domain = domain, recordId =record_id, subDomain = sub_domain, value = value, recordType = record_type, recordLine = line)
    #替换为标准格式
    # {
    #   "result":True,
    #   "message":{...}
